...
class ModelService:
    @staticmethod
    def create_model(db: Session, model_in: ModelCreate) -> ResponseModel[ModelResp]:
        # 检查名称是否已存在
        db_model = model.get_by_name(db, name=model_in.name)
        if db_model:
            raise HTTPException(
    ...
    @staticmethod
    def update_model(db: Session, model_in: ModelUpdate) -> ResponseModel[ModelResp]:
        # 检查是否存在
        ...
        # 检查名称是否已存在（排除自己）
        name_exists = model.get_by_name(db, name=model_in.name)
    ...
    @staticmethod
    def delete_model(db: Session, id: int) -> ResponseModel[ModelResp]:
    ...
    @staticmethod
    def get_model(db: Session, id: int) -> ResponseModel[ModelResp]:
        db_model = model.get(db, id=id)
        if not db_model:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    def get_models_by_type(
        db: Session, type: int, status: Optional[int] = None
    ) -> ResponseModel[List[ModelResp]]:
        models = model.get_multi_by_type_and_status(db, type=type, status=status)
        # 构建响应列表，包含 API 密钥信息
        model_resps = []
        for m in models:
    ...
    @staticmethod
    def get_model_page(db: Session, page: ModelPageReq) -> ResponseModel[PageResult[ModelResp]]:
        items, total = model.get_page(db, page=page)
        # 构建响应列表，包含 API 密钥信息
        model_resps = []
        for m in items:
            model_resp = ModelResp.model_validate(m)
